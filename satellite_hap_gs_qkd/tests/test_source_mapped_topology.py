...
from topology.source_adapters import HAP_QKD_REPO, SATELLITE_QKD_REPO
...
def test_source_repository_labels_are_corrected() -> None:
    ...
    sat_gs_sources = {
        link.metadata["source_repository"]
        for link in scenario.get_links_by_type(LinkType.SAT_GS)
    }
    hap_gs_sources = {
        link.metadata["source_repository"]
        for link in scenario.get_links_by_type(LinkType.HAP_GS)
    }
    sat_hap_sources = {
        link.metadata["source_repository"]
...
def test_directional_queries_and_milp_input() -> None:
    ...
    assert len(topology.get_outgoing_links("GS_TIMMINS")) == 2
    assert len(topology.get_incoming_links("GS_TIMMINS")) == 2
